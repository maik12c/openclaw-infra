...
**Voice transcription:** OpenAI Whisper (`whisper-1`) via the same OpenAI API key.

| Provider | Model | Auth | Endpoint |
|----------|-------|------|----------|
| `openai-codex` | `gpt-5.4` | OAuth (ChatGPT Plus) | `wss://chatgpt.com/backend-api/codex/responses` |
| `openai` | `gpt-5.4-mini` (fallback), `gpt-5.4-nano` | API key | `https://api.openai.com/v1` |
| `openai` | `whisper-1` (voice) | API key (same) | `https://api.openai.com/v1` |
...
| **OpenAI API key** | **Fallback model (`gpt-5.4-mini`) + voice transcription (Whisper)** | platform.openai.com/api-keys |
